...
// click ハンドラ内で時刻比較する方式が最も確実。
let _pendingDblclickItemId = null; // mousedown 方式の残骸（使用しない）
let _lastClickedItemId     = null;
...
function bindEvents() {
  ...
  elMainGrid.addEventListener('click', async e => {
    ...
    // ── click-timer 方式のダブルクリック判定 ──────────────────
    // native dblclick は DOM 再構築後に発火しないブラウザがあるため、
    // click ハンドラ内で「同一品目への連続クリック + 時刻差」で判定する。
    if (itemId === _lastClickedItemId && now - _lastClickTime < DBLCLICK_MS) {
      // ダブルクリック: 編集モードへ
      _lastClickedItemId = null;
      _lastClickTime     = 0;
      const items = adminState.itemsCache[adminState.selectedTruckId] ?? [];

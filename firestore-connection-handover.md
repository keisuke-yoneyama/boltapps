...
| `tempBoltMap` | object | 仮ボルトマッピング設定（旧式） |
...
| フィールド | 型 | 説明 |
|---|---|---|
| `tempBoltSetting` | string | 仮ボルト設定方式。本柱は常に`"none"` |
| `shopTempBoltCount` | number \| null | 継手単位の工場仮ボルト本数 |
| `shopTempBoltSize` | string \| null | 継手単位の工場仮ボルトサイズ |
